...
<script type="text/javascript">
    ...
    {
        $('#input_import_file').attr('accept','*.csv,*.xls');
        $('#select_page').on('change',function(){
            $('#input_current_page').val($(this).val());
            $('#submit_form').click();
            //window.location.assign('{$pagelink}' + $(this).val() + '&pagination=' + $('#input_select_pagination').val()) + params;
        });
        $('#input_select_pagination').on('change',function(){
            $('#input_current_page').val('0');
            $('#submit_form').click();
